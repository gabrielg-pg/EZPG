...
import { createInvestmentAsset, createInvestmentTransaction, updateInvestmentAsset } from "@/app/actions/investment-actions"
    ...
    startTransition(async () => {
      try {
        const saved = editingAsset
          ? await updateInvestmentAsset({ id: editingAsset.id, name: form.name, ticker: form.ticker, category: form.category, institution: form.institution, initialValue: value, currentValue: value })
          : await createInvestmentAsset({ name: form.name, ticker: form.ticker, assetType: form.category, category: form.category, institution: form.institution, initialValue: value, currentValue: value, currency: "BRL" })
        const nextAsset = { id: saved.id, name: saved.name, ticker: saved.ticker, category: saved.category, institution: saved.institution, initial_value: saved.initialValue, current_value: saved.currentValue }
        setAssets((items) => editingAsset ? items.map((item) => item.id === nextAsset.id ? nextAsset : item) : [...items, nextAsset])
        setForm({ name: "", ticker: "", category: "Renda Fixa", institution: "", initialValue: "" }); setEditingAsset(null); setOpen(false)
